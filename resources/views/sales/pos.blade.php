                ...
                <form>
                  ...
                  <div class="filter-box">
                    <validation-provider name="Customer" rules="required" v-slot="{ valid, errors }">
                      <label>{{ __('translate.Member') }} <span class="field_required">*</span></label>
                      <v-select @input="Selected_Customer" v-model="sale.client_id"
                        placeholder="{{ __('translate.Choose_Member') }}" :reduce="username => username.value"
                  ...
                  <!-- Spouse and Children -->
                <div v-if="familyMembers && familyMembers.length > 0" class="filter-box">
                    <label>Spouse and Children</label>
                    <v-select v-model="selectedFamilyMember" :options="familyMembers.map(member => ({ label: member, value: member }))"></v-select>
                </div>


                <div class="filter-box warehouse-dropdown">
                    <!-- warehouse -->
                  <div class="filter-box">
  ...
  <script>
      ...
      var app = new Vue({
        ...
        data: {
            ...
            familyMembers: [], // list of family members (spouse and children)
        ...
        mounted() {
            // Assuming 'warehouses' contains the list of warehouses
            if (this.warehouses.length > 0) {
                // Auto-select the first warehouse
                this.sale.warehouse_id = this.warehouses[0].id;

                // Fetch products for the first warehouse
        ...
        //---------------------------------
         ...
         Submit_Payment() {
          ...
          // Inside your Vue component methods
            ...
            //   // Fetch spouse and children data based on this.sale.client_id
            //   axios.get(`/customers/family-members/${this.sale.client_id}`)
            //       .then(response => {
            //       // Assuming the response.data contains the entire response object
            //       const familyMembers = response.data.familyMembers;

            //       // Extract names from familyMembers object and filter out empty values
            //       const familyMemberNames = Object.values(familyMembers).filter(name => name.trim() !== "");

            //       // Assign names directly to the dropdown options
            //       this.familyMembers = familyMemberNames;

            //       console.log(this.familyMembers); // Check if this correctly formats the options
            //       })
            //       .catch(error => {
        ...
        //             // Format the spouse and children details for display
        //             const formattedFamilyMembers = Object.entries(familyMembers)
        //                 .map(([key, value]) => {
        ...
        //             // Assign formatted family members to the dropdown options
        //             this.familyMembers = formattedFamilyMembers;
        ...
        Selected_Customer(value) {
            if (value === null) {
                this.sale.client_id = "";
                this.selectedFamilyMember = null; // Reset the selected family member when changing the customer
                return;
            }
            ...
            axios.get(`/customers/family-members/${this.sale.client_id}`)
                .then(response => {
                    ...
                    // Check if the selectedFamilyMember is in the list of family members
                    if (!this.familyMembers.includes(this.selectedFamilyMember)) {
                        this.selectedFamilyMember = null; // Reset to null if not found
                    }
            ...
            NProgress.set(0.1);
              ...
              axios
                .post("/pos/create_pos", {
                  ...
                  enjoyed_by: this.selectedFamilyMember,
