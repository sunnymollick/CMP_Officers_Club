...
<div class="breadcrumb">
  <h1>{{ __('translate.CookedProductStore') }}</h1>
</div>
      ...
      <form @submit.prevent="Submit_Sale">
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="col-md-4 hide-date-field">
            <!-- Date -->
              <div class="col-md-4">
              ...
              <div class="form-group col-md-4">
                <validation-provider name="Customer" rules="required" v-slot="{ valid, errors }">
                  <label>{{ __('translate.Service_Product') }} <span class="field_required">*</span></label>
                  <v-select @input="Selected_Customer" v-model="sale.client_id" :reduce="label => label.value"
                    placeholder="{{ __('translate.Choose_Service_Product') }}"
                    :options="service_products.map(service_products => ({label: service_products.  name, value: service_products.id}))"></v-select>
                  <span class="error">@{{ errors[0] }}</span>
                </validation-provider>
              </div>

            <!-- Your template -->
            <div class="form-group col-md-4 hide-warehouse-field">
                <!-- warehouse -->
              <div class="form-group col-md-4">
  ...
  Vue.component('validation-observer', VeeValidate.ValidationObserver);
    ...
    var app = new Vue({
        ...
        data: {
          ...
          service_products: @json($service_products),
          client: {},
          products: [],
          details: [],
          detail: {},
        ...
        if (this.warehouses.length > 0) {
            ...
            this.sale.warehouse_id = firstWarehouseId;

            // Fetch products for the first warehouse
            this.Get_Products_By_Warehouse(firstWarehouseId);
        }
        },
    ...
    Create_Sale() {
      if (this.verifiedForm()) {
        ...
        NProgress.set(0.1);
          ...
          axios
            .post("/products/storeServiceProductCalculation", {
              date: this.sale.date,
            ...
            .then(response => {
              ...
              window.location.href = '/products/service_product_calculation';
            })
            .catch(error => {
              NProgress.done();
              this.paymentProcessing = false;
