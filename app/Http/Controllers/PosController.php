...
class PosController extends Controller
{
    ...
    public function CreatePOS(Request $request)
    {
        ...
        $enjoyedBy = $request->enjoyed_by;

        $item = \DB::transaction(function () use ($request,$enjoyedBy) {
            $helpers = new helpers();
            $order = new Sale;
            ...
            if ($enjoyedBy != "") {
                $order->enjoyed_by = $enjoyedBy["value"];
            }
            ...
            foreach ($data as $key => $value) {
                ...
                $service_product = ServiceProduct::where('parent_product_id', $value["product_id"])->get();
                // dd($service_product);
                // dd($value['quantity']);
                if ($service_product != null) {
                    foreach ($service_product as $key => $val) {
                        // dd($value);
                        $unit = Unit::where('id', $val['purchase_unit_id'])
                        ->first();
                        if ($val['product_variant_id']) {
                            $product_warehouse = product_warehouse::where('warehouse_id', $val['warehouse_id'])
                            ->where('product_id', $val['child_product_id'])->where('product_variant_id', $val['product_variant_id'])
                            ->first();
                            ...
                            if ($unit && $product_warehouse) {
                                if ($unit->operator == '/') {
                                    $product_warehouse->qte -= $value["quantity"] * ($val['quantity'] / $unit->operator_value);
                                } else {
                                    $product_warehouse->qte -= $value["quantity"] * ($val['quantity'] * $unit->operator_value);
                                }
                            ...
                            $product_warehouse = product_warehouse::where('warehouse_id', $val['warehouse_id'])
                            ->where('product_id', $val['child_product_id'])
                            ->first();
                            // dd($value["quantity"]);
                            if ($unit && $product_warehouse) {
                                if ($unit->operator == '/') {
