...
class ProductsController extends Controller
{
            ...
            if($user_auth->is_all_warehouses){
                ...
                foreach ($products as $product) {
                    ...
                    if ($user_auth->can('products_edit')){
                        $item['action'] .= '<a class="dropdown-item" href="/products/products/' .$product->id. '"><i class="nav-icon i-Edit text-success font-weight-bold m3-2"></i> ' .trans('translate.view_product').'</a>';
                    }
                    if ($user_auth->can('products_edit')){
    ...
    public function show($id)
    {
        try {
            ...
            if ($user_auth->can('products_edit')) {
                ...
                $cookedProduct = ServiceProduct::where('parent_product_id', $Product->id)->get();
    ...
    public function service_product_calculation()
    {
        try {
            $user_auth = auth()->user();
            if ($user_auth->can('sales_add')) {
                ...
                $service_products = Product::where('deleted_at', '=', null)->where('is_active', 1)
                ...
                return view(
                    'products.service_product_calculation',
                    [
    ...
    public function storeServiceProductCalculation(Request $request)
    {
        ...
        if ($user_auth->can('sales_add')) {
            ...
            \DB::transaction(function () use ($request) {
                ...
                foreach ($data as $key => $value) {
                    ...
                    ->first();
                    $orderDetails[] = [
                        'parent_product_id' => $request->client_id,
                        'child_product_id' => $value['product_id'],
                        'warehouse_id' => $request->warehouse_id,
                        'purchase_unit_id' =>  $value['sale_unit_id'] ? $value['sale_unit_id'] : NULL,
                ...
                ServiceProduct::insert($orderDetails);
            }, 10);
