...
class ReportController extends Controller
{
    ...
    //----- Purchase report 2 -------\\
    public function purchase_report_2(Request $request)
    {

        $user_auth = auth()->user();
		if ($user_auth->can('purchase_reports')){

            return view('reports.purchase_report_2');
    ...
    public function categoryMonthlyPurchaseReport(Request $request){
        try {
            $month = $request->input('month');
            $month_name = $request->input('month_name');
            $month_start = strtotime('first day of ' . $month_name, time());
            $month_end = strtotime('last day of ' . $month_name, time());
            ...
            ->join('purchases', 'purchases.id', 'purchase_details.purchase_id')
            ->selectRaw('DATE(purchases.date) as purchase_date, categories.name as category_name, SUM(purchases.GrandTotal) as total_amount')
            ->whereBetween('purchases.date', [date('Y-m-01', $month_start), date('Y-m-t', $month_end)])
            ->groupBy('purchase_date', 'categories.name')
            ->get();

            // dd($products);
            // Assuming $products is your collection

            // Get unique values from the 'category_name' column
            $uniqueCategories = $products->pluck('category_name')->unique();

            // dd($uniqueCategories);


            $view = View::make('reports.monthly_purchase_report', compact('month', 'month_name', 'products', 'uniqueCategories'))->render();
